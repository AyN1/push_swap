...
{
	if (!(*str_n == '+' || *str_n == '-' || ft_isdigit(*str_n))) //Check if the first character of the input string does not contain a sign or a digit
		return (1);
    if ((*str_n == '+' || *str_n == '-') && !ft_isdigit(str_n[1])) //Check if the first character of the input string contains a sign, but the second character does not contain a digit
		return (1);
	...
	{
		if (!ft_isdigit(*str_n)) //Check if the next character in the string is not a digit
			return (1);
	}
	return (0);
}
